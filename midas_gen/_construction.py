...
class CS:
    ...
    class STAGE:
        ...
        @classmethod
        def delete(cls):
            """Deletes all construction stages from the database and GEN NX"""
            cls.clear()
...
#-----------------------------------------------------------------------------------------------------------------------------------
    ...
    class TimeLoad:
        ...
        @classmethod
        def create(cls):
            """Creates time loads in the GEN NX"""
            return MidasAPI("PUT", "/db/tmld", cls.json())
        
        @classmethod
        def get(cls):
            """Gets time loads data from the GEN NX"""
            return MidasAPI("GET", "/db/tmld")
        
        @classmethod
        def sync(cls):
            """Updates the TimeLoad class with data from the GEN NX"""
            cls.timeloads = []
            response = cls.get()
