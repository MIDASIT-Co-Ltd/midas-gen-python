...
class utils:
    ...
    class Alignment:
        ...
        def modifyNXModel(initial_align:utils.Alignment,final_align:utils.Alignment,bElement:bool=True,bUpdateModel=True,bSync=True):
            '''
            Modifies GEN NX model as per new alignment.  
            ...
            :type bElement: bool
            '''
            from midas_gen import Node,Element,MidasAPI,nodeByID
            if bSync:
                Node.sync()
    ...
    def RC_Grillage(span_length = 20, width = 8, support:Literal['fix','pin']='fix', dia_no=2,start_loc = [0,0,0], girder_depth = 0, girder_width = 0, girder_no = 0, 
        ...
        Use Model.create() to create model in GEN NX
        ...
        """
        from midas_gen import Model,Material,Section,Offset,nodesInGroup,Element,Boundary,Load,elemsInGroup,Node,Group
        import math
